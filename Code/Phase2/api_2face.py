...
class PRN_2face:
    ...
    def __init__(self, is_dlib = False, prefix = '.'):
        ...
        if is_dlib:
            import dlib
            detector_path = os.path.join(prefix, 'Phase2/Data/net-data/mmod_human_face_detector.dat')
        ...
        prn_path = os.path.join(prefix, 'Phase2/Data/net-data/256_256_resfcn256_weight')
        ...
        # uv file
        self.uv_kpt_ind = np.loadtxt(prefix + '/Phase2/Data/uv-data/uv_kpt_ind.txt').astype(np.int32) # 2 x 68 get kpt
        self.face_ind = np.loadtxt(prefix + '/Phase2/Data/uv-data/face_ind.txt').astype(np.int32) # get valid vertices in the pos map
        self.triangles = np.loadtxt(prefix + '/Phase2/Data/uv-data/triangles.txt').astype(np.int32) # ntri x 3
    ...
    def process(self, input, image_info = None):
        ...
        if image_info is not None:
            ...
            if len(detected_faces) != 2:
                print("Can't detect two faces here!")
                return None
            print("Number of faces Detected",len(detected_faces))
            # Edit that accommodates working for two faces
            poses = []
